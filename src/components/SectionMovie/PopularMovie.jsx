...
function PopularMovie() {
    ...
    const API_URL_MOVIES_POPULAR = `${BASE_URL}/movie/popular?api_key=${api_key}`;
    const [moviesPopular, setMoviesPopular] = useState([]);
    const [pagePopular, setPagePopular] = useState(0);
    ...
    useEffect(() => {
        fetch(API_URL_MOVIES_POPULAR)
            .then((res) => res.json())
            .then((data) => setMoviesPopular(data.results))
            .finally(() => setLoading(false));
    }, []);

    const currentMoviesPopular = moviesPopular.slice(
        pagePopular * moviesPerPage,
        (pagePopular + 1) * moviesPerPage
    );

    const handleNextPopular = () => {
        if ((pagePopular + 1) * moviesPerPage < moviesPopular.length) {
            setPagePopular((prev) => prev + 1);
        }
    };
    const handlePrevPopular = () => {
        if (pagePopular > 0) setPagePopular((prev) => prev - 1);
    };
    ...
    return (
        <div className="p-5 my-5">
            <div className="flex flex-row space-x-20">
                {currentMoviesPopular.map((movie) => (
                    <div
                        key={movie.id}
            ...
            <div className="flex justify-between mt-5">
                ...
                <button
                    ...
                    disabled={
                        (pagePopular + 1) * moviesPerPage >= moviesPopular.length
                    }
